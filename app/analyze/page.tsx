...
import {
  Upload, FileAudio, RefreshCw, Activity, Clock, Music, Zap, Hash,
  ChevronRight, ChevronLeft, Loader2, Check, AlertCircle, Download,
  CheckSquare, Square,
} from "lucide-react"
...
function SpotifyMiniPicker({ onImportDone }: { onImportDone: () => void }) {
  ...
  const [importDone, setImportDone] = useState<{ queued: number; ready: number } | null>(null)
  ...
  useEffect(() => {
    ...
    }).catch(() => setIsConnected(false)).finally(() => setIsChecking(false))
  }, [])
  ...
  async function openPl(pl: SpotifyPlaylist) {
    ...
    const data = await fetch(`/api/spotify/playlists/${pl.id}`).then(r => r.json())
  ...
  async function handleImport() {
    ...
    setIsImporting(true)
    const tracksToImport = tracks.filter(t => selected.has(t.id)).map(t => ({
      id: t.id, name: t.name, artist: t.artist, previewUrl: t.previewUrl,
      bpm: t.bpm, key: t.key, energy: t.energy, durationMs: t.durationMs,
    }))
    try {
  ...
  if (!isConnected) return (
    <div className="flex flex-col items-center justify-center gap-4 py-8 text-center">
      ...
      <p className="text-sm text-muted-foreground">Connect Spotify to import tracks for analysis</p>
      <a
    ...
    <div className="flex flex-col items-center justify-center gap-3 py-8 text-center">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          {importDone.queued > 0 && `${importDone.queued} tracks being analyzed`}
          {importDone.ready > 0 && (importDone.queued > 0 ? ' · ' : '') + `${importDone.ready} added instantly`}
        </p>
      </div>
      <p className="text-[10px] text-muted-foreground/50">Refreshing library…</p>
    </div>
  )
  ...
  if (!openPlaylist) return (
    <div className="space-y-1">
      <p className="text-[10px] text-muted-foreground px-1 mb-2">Select a playlist to import tracks</p>
      {isLoadingPlaylists ? (
        <div className="flex items-center justify-center h-20"><Loader2 className="w-4 h-4 text-[#1DB954] animate-spin" /></div>
      ) : playlists.map(pl => {
        const thumb = smallImg(pl.images)
        return (
          <button key={pl.id} onClick={() => openPl(pl)}
          ...
          >
            <div className="w-8 h-8 flex-shrink-0 rounded overflow-hidden bg-white/[0.05]">
              {thumb ? <img src={thumb} alt="" className="w-full h-full object-cover" loading="lazy" width={32} height={32} /> : <Music className="w-4 h-4 text-muted-foreground/25 m-2" />}
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-xs font-medium truncate text-foreground/80 group-hover:text-white">{pl.name}</div>
              <div className="text-[9px] text-muted-foreground/50">{pl.tracks.total} songs</div>
            </div>
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-2">
        <button onClick={() => { setOpenPlaylist(null); setTracks([]); setSelected(new Set()) }}
          className="w-6 h-6 rounded flex items-center justify-center hover:bg-white/10 transition-colors">
          <ChevronLeft className="w-3.5 h-3.5 text-muted-foreground" />
        </button>
        <span className="text-xs font-semibold truncate flex-1">{openPlaylist.name}</span>
        <button onClick={() => allSel ? setSelected(new Set()) : setSelected(new Set(tracks.map(t => t.id)))}
      ...
      <div className="max-h-[300px] overflow-y-auto space-y-0.5">
        {isLoadingTracks ? (
          ...
          return (
            <div key={track.id} onClick={() => toggle(track.id)}
              className={cn(
                'flex items-center gap-2 px-2 py-1.5 rounded-lg cursor-pointer transition-colors',
                isSel ? 'bg-[#1DB954]/[0.07]' : 'hover:bg-white/[0.03]'
              )}>
              <button className={cn('flex-shrink-0 transition-colors', isSel ? 'text-[#1DB954]' : 'text-muted-foreground/30')}>
              ...
              <div className="w-7 h-7 flex-shrink-0 rounded overflow-hidden bg-white/[0.05]">
                {track.albumArt
                  ? <img src={track.albumArt} alt="" className="w-full h-full object-cover" loading="lazy" width={28} height={28} />
                  : <Music className="w-3 h-3 text-muted-foreground/30 m-2" />}
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-[10px] font-medium truncate">{track.name}</div>
                <div className="text-[9px] text-muted-foreground/60 truncate">{track.artist}</div>
              </div>
              <span className="text-[9px] font-mono text-muted-foreground/40 flex-shrink-0">{fmtMs(track.durationMs)}</span>
            </div>
          )
        })}
      </div>
      ...
      >
        {isImporting ? <><Loader2 className="w-3 h-3 animate-spin" /> Importing…</> : <><Download className="w-3 h-3" /> Import {selected.size > 0 ? `${selected.size} tracks` : ''} to Library</>}
      </button>
    </div>
  )
}
            ...
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardTitle className="text-base">
                  {uploadTab === 'file' ? 'Upload & Analyze' : 'Import from Spotify'}
                </CardTitle>
                <CardDescription>
                  {uploadTab === 'file'
                ...
                {uploadTab === 'file' ? (
                  <>
                    ...
                    >
                      ...
                      <div>
                        <Input type="file" accept="audio/*" onChange={e => e.target.files?.[0] && setSelectedFile(e.target.files[0])} className="hidden" id="file-upload" />
                    ...
                    {selectedFile && (
                      <Button className="mt-4 w-full" onClick={handleAnalyze} disabled={uploading}>
                        {uploading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Uploading…</> : 'Start Analysis'}
                      </Button>
                    )}
                  </>
                ) : (
                  <SpotifyMiniPicker onImportDone={() => { fetchTracks(); setUploadTab('file') }} />
                )}
              </CardContent>
            </Card>
              ...
              <CardContent className="max-h-[500px] overflow-y-auto p-0">
                {tracks.length > 0 ? (
                    ...
                    {tracks.map(track => (
                      ...
                      >
                        ...
                        <div className="flex gap-3 shrink-0 px-2 text-xs text-muted-foreground items-center">
                          <span className="w-12 text-right whitespace-nowrap">{track.bpm} <span className="text-[10px]">BPM</span></span>
                          <span className="w-8 text-center font-bold px-1.5 py-0.5 rounded text-white shadow-sm" style={{ backgroundColor: getCamelotColor(track.key) }}>{track.key}</span>
                          <span className="w-8 text-center whitespace-nowrap">E: {track.energy.toFixed(1)}</span>
                          <span className="w-10 text-right whitespace-nowrap">{formatDuration(track.duration)}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="p-8 text-center text-muted-foreground">
                    {loadingTracks ? "Loading tracks..." : "No analyzed tracks found. Upload music or import from Spotify!"}
                  </div>
                )}
              </CardContent>
            </Card>
          ...
          <div>
            <Card className="h-full">
              <CardHeader>
                ...
                <CardDescription>
                  {selectedTrack ? "Detailed metrics for the selected track" : "Select a track to view analysis"}
                </CardDescription>
              </CardHeader>
              <CardContent>
                {selectedTrack ? (
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                      ...
                      <div className="relative overflow-hidden rounded-xl border p-5 shadow-sm text-white"
                        style={{ background: `linear-gradient(135deg, ${getCamelotColor(selectedTrack.key)} 0%, ${getCamelotColor(selectedTrack.key)}88 100%)` }}>
                        <div className="absolute top-0 right-0 p-3 opacity-20 text-black"><Hash className="w-16 h-16" /></div>
                      ...
                      <div className="relative overflow-hidden rounded-xl border bg-gradient-to-br from-card to-card/50 p-5 shadow-sm">
                        <div className="absolute -bottom-2 -right-2 p-3 opacity-10 text-orange-500"><Zap className="w-20 h-20" /></div>
                        <div className="font-semibold mb-2 text-[10px] text-muted-foreground uppercase tracking-widest flex items-center gap-1.5">
                      ...
                      <div className="relative overflow-hidden rounded-xl border bg-gradient-to-br from-card to-card/50 p-5 shadow-sm">
                        <div className="absolute top-0 right-0 p-3 opacity-10"><Clock className="w-16 h-16" /></div>
                        <div className="font-semibold mb-2 text-[10px] text-muted-foreground uppercase tracking-widest flex items-center gap-1.5">
                        ...
                        {selectedTrack.waveform?.length > 0 ? (
                          ...
                          <div className="flex items-end h-full w-full gap-[2px] opacity-70">
                            {Array.from({ length: 80 }).map((_, i) => {
                              ...
                              let envelope = progress < 0.1 ? progress * 10 : progress > 0.9 ? (1 - progress) * 10 : 1.0 + Math.sin(progress * Math.PI * 4) * 0.2
                              const h = Math.min(100, Math.max(5, (20 + energyMulti * envelope) + Math.random() * 20))
                              return <div key={i} className="bg-primary/60 rounded-t-sm w-full" style={{ height: `${h}%` }} />
                            })}
                          </div>
                        )}
                      </div>
