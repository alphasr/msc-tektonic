...
import { useSpotifyPlayer, spotifyUrlToUri } from "@/hooks/useSpotifyPlayer";

// Spotify imports have no downloadable audio (Spotify removed preview URLs
// for new apps) — they stream via the Web Playback SDK instead of Howler.
const isSpotifyStreamTrack = (track: Track | null): track is Track =>
  !!track && !track.audioUrl && !!track.spotifyUrl;
...
export default function Home() {
  ...
  const [hotCuesA, setHotCuesAState] = useState<(number | null)[]>([
    null,
    null,
    null,
    null,
  ]);
  const [hotCuesB, setHotCuesBState] = useState<(number | null)[]>([
    null,
    null,
    null,
    null,
  ...
  const setHotCuesA = (
    updater: ((p: (number | null)[]) => (number | null)[]) | (number | null)[],
  ) => {
  ...
  // Single Web Playback SDK instance for the whole page (one Spotify Connect
  // device) — shared by the deck transport handlers and the mini player.
  ...
  // ── Transport handlers ──
  // Spotify streams: whether this deck's track is the one on the SDK device.
  // Only then do pause/seek calls (which act on the current stream) apply.
  const isCurrentSpotifyStream = (track: Track | null): boolean =>
    isSpotifyStreamTrack(track) &&
    spotifyUrlToUri(track.spotifyUrl) === spotifyPlayer.currentTrackUri;

  const handleSpotifyPlay = (deckKey: "deckA" | "deckB", track: Track) => {
    if (spotifyPlayer.isPremiumRequired) {
      alert("Spotify Premium is required to stream full tracks.");
      return;
    }
    ...
    const uri = spotifyUrlToUri(track.spotifyUrl);
    if (!uri) return;
    lastUserActionRef.current = Date.now();
    if (uri === spotifyPlayer.currentTrackUri) {
      spotifyPlayer.resume();
    } else {
      spotifyPlayer.playTrack(uri).then((ok) => {
        if (ok) return;
        alert(
          `Could not start "${track.title}" on Spotify — check your connection and Premium status.`,
        );
        setMixerState((prev) => ({
          ...prev,
  ...
  const handlePlay = (deck: "A" | "B") => {
    ...
    const track = mixerState[deckKey].track;
    if (isSpotifyStreamTrack(track)) {
      handleSpotifyPlay(deckKey, track);
      return;
    }
  ...
  const handlePause = (deck: "A" | "B") => {
    ...
    lastUserActionRef.current = Date.now();
    if (isSpotifyStreamTrack(track)) {
      if (isCurrentSpotifyStream(track)) spotifyPlayer.pause();
    } else {
      const deckRef = deck === "A" ? deckARef.current : deckBRef.current;
      if (!deckRef) return;
      deckRef.pause();
  ...
  const handleStop = (deck: "A" | "B") => {
    ...
    lastUserActionRef.current = Date.now();
    if (isSpotifyStreamTrack(track)) {
      if (isCurrentSpotifyStream(track)) {
        spotifyPlayer.pause();
  ...
  const handleSync = (deck: "A" | "B") => {
    setMixerState((prev) => {
      ...
      // Metadata-only imports can have bpm 0 — syncing would set rate to 0/Infinity
      if (!src.track.bpm || !tgt.track.bpm) return prev;
  ...
  const handleSeek = (deck: "A" | "B", time: number) => {
    const track = mixerState[deck === "A" ? "deckA" : "deckB"].track;
    if (isSpotifyStreamTrack(track)) {
      if (isCurrentSpotifyStream(track)) spotifyPlayer.seek(time);
    } else {
      const deckRef = deck === "A" ? deckARef.current : deckBRef.current;
      deckRef?.seek(time);
    }
    ...
    if (cues[index] !== null) {
      ...
      const currentTime =
        deck === "A"
          ...
          : (deckBRef.current?.getCurrentTime() ??
            mixerState.deckB.currentTime);
      setCues(
        (prev) =>
          prev.map((c, i) => (i === index ? currentTime : c)) as (
            number | null
          )[],
      );
    }
  ...
  const handleHotCueClear = (deck: "A" | "B", index: number) => {
    ...
    setCues(
      (prev) =>
        prev.map((c, i) => (i === index ? null : c)) as (number | null)[],
    );
  };
  ...
  // ── Auto transition ──
  const autoTransitionActiveRef = useRef(false);
  const handleAutoTransition = async () => {
    // A second trigger mid-fade would run two crossfade intervals against each other
    if (autoTransitionActiveRef.current) return;
    autoTransitionActiveRef.current = true;
    ...
    if (!targetTrack || !activeTrack) {
      autoTransitionActiveRef.current = false;
      return;
    }

    const targetDeckRef =
      targetDeck === "A" ? deckARef.current : deckBRef.current;
  ...
  const setupMIDIHandlers = () => {
    ...
    // Hardware play buttons are play/pause toggles (Party Mix, XDJ-RR, …)
    const midiPlayToggle = (deck: "A" | "B") => {
      const ref = deck === "A" ? deckARef.current : deckBRef.current;
      if (ref?.isPlaying()) handlePause(deck);
      else handlePlay(deck);
    };

    // Deck A transport
    midi.on("deckA_play", () => midiPlayToggle("A"));
    midi.on("deckA_pause", () => handlePause("A"));
    midi.on("deckA_stop", () => handleStop("A"));
    midi.on("deckA_cue", () => handleCue("A"));
    // Deck B transport
    midi.on("deckB_play", () => midiPlayToggle("B"));
    ...
    midi.on("deckA_low", (v) =>
      setMixerState((prev) => {
        const s = {
          ...prev,
          deckA: { ...prev.deckA, eq: { ...prev.deckA.eq, low: v } },
        };
        deckARef.current?.setEQ(s.deckA.eq);
        return s;
      }),
    );
    midi.on("deckA_mid", (v) =>
      setMixerState((prev) => {
        const s = {
          ...prev,
          deckA: { ...prev.deckA, eq: { ...prev.deckA.eq, mid: v } },
        };
        deckARef.current?.setEQ(s.deckA.eq);
        return s;
      }),
    );
    midi.on("deckA_high", (v) =>
      setMixerState((prev) => {
        const s = {
          ...prev,
          deckA: { ...prev.deckA, eq: { ...prev.deckA.eq, high: v } },
        };
        deckARef.current?.setEQ(s.deckA.eq);
        return s;
      }),
    ...
    midi.on("deckB_low", (v) =>
      setMixerState((prev) => {
        const s = {
          ...prev,
          deckB: { ...prev.deckB, eq: { ...prev.deckB.eq, low: v } },
        };
        deckBRef.current?.setEQ(s.deckB.eq);
        return s;
      }),
    );
    midi.on("deckB_mid", (v) =>
      setMixerState((prev) => {
        const s = {
          ...prev,
          deckB: { ...prev.deckB, eq: { ...prev.deckB.eq, mid: v } },
        };
        deckBRef.current?.setEQ(s.deckB.eq);
        return s;
      }),
    );
    midi.on("deckB_high", (v) =>
      setMixerState((prev) => {
        const s = {
          ...prev,
          deckB: { ...prev.deckB, eq: { ...prev.deckB.eq, high: v } },
        };
        deckBRef.current?.setEQ(s.deckB.eq);
        return s;
      }),
    ...
    midi.on("jogwheelA", (v) => {
      if (deckARef.current?.isPlaying()) {
        deckARef.current.seek(
          Math.max(0, deckARef.current.getCurrentTime() + v * 0.1),
        );
      }
    });
    midi.on("jogwheelB", (v) => {
      if (deckBRef.current?.isPlaying()) {
        deckBRef.current.seek(
          Math.max(0, deckBRef.current.getCurrentTime() + v * 0.1),
        );
      }
    });
  ...
  const startProgressTracking = () => {
    ...
    progressInterval.current = setInterval(() => {
      setMixerState((prev) => {
        ...
        // Streaming decks have no Howler state — advance the clock locally;
        // the Spotify sync effect corrects drift from real SDK positions.
        if (isSpotifyStreamTrack(prev.deckA.track)) {
          if (prev.deckA.isPlaying)
            aUpdates = { currentTime: prev.deckA.currentTime + 0.05 };
        } else if (deckARef.current) {
          const t = deckARef.current.getCurrentTime();
          if (Math.abs(t - prev.deckA.currentTime) > 0.05)
            aUpdates = { currentTime: t };
          if (now - lastUserActionRef.current > 300) {
            const playing = deckARef.current.isPlaying();
            if (playing !== prev.deckA.isPlaying)
              aUpdates = { ...(aUpdates ?? {}), isPlaying: playing };
          }
          if (
            prev.deckA.track &&
            t >= prev.deckA.track.duration - 0.1 &&
            prev.deckA.isPlaying
          ) {
            aUpdates = {
              ...(aUpdates ?? {}),
              isPlaying: false,
          ...
          // Loop monitoring (only while this deck is actually playing)
        ...
        if (isSpotifyStreamTrack(prev.deckB.track)) {
          if (prev.deckB.isPlaying)
            bUpdates = { currentTime: prev.deckB.currentTime + 0.05 };
        } else if (deckBRef.current) {
          const t = deckBRef.current.getCurrentTime();
          if (Math.abs(t - prev.deckB.currentTime) > 0.05)
            bUpdates = { currentTime: t };
          if (now - lastUserActionRef.current > 300) {
            const playing = deckBRef.current.isPlaying();
            if (playing !== prev.deckB.isPlaying)
              bUpdates = { ...(bUpdates ?? {}), isPlaying: playing };
          }
          if (
            prev.deckB.track &&
            t >= prev.deckB.track.duration - 0.1 &&
            prev.deckB.isPlaying
          ) {
            bUpdates = {
              ...(bUpdates ?? {}),
              isPlaying: false,
  ...
  const loadTrack = async (track: Track, deck: "A" | "B") => {
    ...
    const deckRef = deck === "A" ? deckARef.current : deckBRef.current;
    if (!deckRef) {
      alert(`Deck ${deck} is not ready. Please refresh.`);
      return;
    }

    try {
      if (deckRef.isPlaying()) deckRef.stop();
      // A Spotify stream previously loaded on this deck keeps playing on the
      // SDK device unless explicitly paused.
      if (isCurrentSpotifyStream(mixerState[deckKey].track)) {
        spotifyPlayer.pause();
      }
      deckRef.off("end");
      deckRef.off("play");
      ...
      // Nothing to load into the Howler deck — playback goes through the
      // Spotify Web Playback SDK via the transport handlers.
      if (isSpotifyStreamTrack(track)) return;
      ...
      alert(
        `Failed to load "${track.title}": ${error?.message ?? "Unknown error"}`,
      );
      setMixerState((prev) => ({
        ...prev,
        [deckKey]: {
  ...
  // ── Live mode helpers ──
  const handleAddSegment = (seg: SegmentSuggestion) =>
    playlistManagerRef.current?.addSegment(seg);
  ...
  const handlePlaySegment = async (seg: PlaylistSegment) => {
    ...
    if (!track) {
      alert(`Track not found: ${seg.trackId}`);
      return;
    }
    ...
    try {
      ...
      setTimeout(
        () => clearInterval(monitor),
        (seg.endTime - seg.startTime) * 1000 + 1000,
      );
    } catch (err: any) {
  ...
  const handlePlayPlaylist = async (pl: Playlist) => {
    ...
    try {
      ...
      alert("Failed to start playlist: " + (err as Error).message);
    }
  };

  const getActiveKey = () => {
    if (mixerState.deckA.isPlaying && mixerState.deckA.track)
      return mixerState.deckA.track.key;
    if (mixerState.deckB.isPlaying && mixerState.deckB.track)
      return mixerState.deckB.track.key;
      ...
      <div className="flex-shrink-0 border-b border-white/[0.04] bg-background/40 px-3 py-1 flex items-center gap-3">
        ...
        {liveMode === "live" && playlistManagerRef.current && (
          <span className="text-[9px] text-muted-foreground/50">
            {playlistManagerRef.current.getPlaylist().segments.length} segments
            queued
          </span>
        )}
        <div className="ml-auto">
          <SpotifyMiniPlayer
            currentTrack={mixerState.deckA.track ?? mixerState.deckB.track}
          />
        </div>
      </div>
          ...
          <Panel defaultSize={65} minSize={40} className="min-h-0 p-2 pb-0">
            <div className="h-full">
              <PanelGroup direction="horizontal" className="h-full">
                ...
                {/* Deck A */}
                <Panel
                  defaultSize={liveMode === "live" ? 20 : 33}
                  minSize={20}
                ...
                {/* Central Mixer */}
                <Panel
                  defaultSize={liveMode === "live" ? 20 : 34}
                  minSize={18}
                  className="min-w-0 overflow-hidden flex flex-col"
                >
                  <CentralMixer
                    ...
                    onDeckAEQChange={(band, v) =>
                      setMixerState((prev) => ({
                        ...prev,
                        deckA: {
                          ...prev.deckA,
                          eq: { ...prev.deckA.eq, [band]: v },
                        },
                      }))
                    }
                    onDeckBEQChange={(band, v) =>
                      setMixerState((prev) => ({
                        ...prev,
                        deckB: {
                          ...prev.deckB,
                          eq: { ...prev.deckB.eq, [band]: v },
                        },
                      }))
                    }
              ...
              <Tabs defaultValue="library" className="h-full flex flex-col">
                ...
                >
                  <NaturalLanguageQuery
                    onLoadTrack={loadTrack}
                    currentTrackId={
                      mixerState.deckA.track?.id ?? mixerState.deckB.track?.id
                    }
                  />
                </TabsContent>
                <TabsContent
