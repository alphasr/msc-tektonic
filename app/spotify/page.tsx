...
function medImg(
  ...
  const sorted = [...images].sort(
    (a, b) =>
      Math.abs((a.width ?? 9999) - 300) - Math.abs((b.width ?? 9999) - 300),
  );
  return sorted[0].url;
}
...
export default function SpotifyPage() {
  ...
  async function loadTracks(playlist: SpotifyPlaylist) {
    ...
    stopPreview();
    try {
      const data = await fetch(`/api/spotify/playlists/${playlist.id}`).then(
        (r) => r.json(),
      );
      if (data.error) throw new Error(data.error);
        ...
        <div className='flex-1 flex flex-col items-center justify-center gap-6 p-8 text-center'>
          ...
          <div>
            ...
            <p className='text-sm text-muted-foreground max-w-sm leading-relaxed'>
              Browse your playlists, select tracks, and import them into your
              TEKTONIC library for analysis and mixing.
            </p>
          </div>
          {error && (
  ...
  const allSelected =
    playlistTracks.length > 0 && selectedCount === playlistTracks.length;
        ...
        <div
          className={cn(
            ...
            openPlaylist
              ...
              : 'flex flex-1 md:w-80 md:flex-none',
          )}
        >
          {/* Header */}
        ...
        {openPlaylist ? (
          <div className='flex-1 flex flex-col overflow-hidden min-w-0'>
            ...
            <div className='flex-shrink-0 flex items-center gap-3 px-4 py-3 border-b border-white/[0.06]'>
              ...
              {/* Select all / none + Import to Library */}
            ...
            {importResults && (
              <div className='flex-shrink-0 mx-4 mt-2 flex items-center gap-3 bg-[#1DB954]/10 border border-[#1DB954]/25 px-3 py-2 rounded-xl text-[11px]'>
                ...
                <span className='text-muted-foreground'>
                  {importResults.queued > 0 &&
                    `${importResults.queued} analyzing`}
                  {importResults.ready > 0 &&
                    (importResults.queued > 0 ? `, ` : '') +
                      `${importResults.ready} added instantly`}
                  {importResults.skipped > 0 &&
                    ` · ${importResults.skipped} already in library`}
                  {importResults.errored > 0 &&
                    ` · ${importResults.errored} failed`}
                </span>
                <button
                    ...
                    >
                      ...
                      <div className='flex-1 min-w-0'>
                        ...
                        <div className='flex items-center gap-1.5 mt-0.5'>
                          ...
                          {!canPreview && (
                            <span className='text-[8px] text-muted-foreground/40 flex-shrink-0 border border-white/10 px-1 rounded'>
                              no preview
                            </span>
                          )}
                        </div>
