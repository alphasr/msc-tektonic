...
export const HOT_CUE_COLORS = ['#ff6a00', '#0078ff', '#00cc44', '#ffcc00'] as const;
    ...
    <div className={cn(
      ...
      <div className="flex items-center justify-between mb-1.5 flex-shrink-0">
        <div className="flex items-center gap-2">
          <div
            className={cn("w-1.5 h-1.5 rounded-full", deck.isPlaying && "playing-dot")}
            style={{ backgroundColor: accentVar, boxShadow: deck.isPlaying ? `0 0 6px ${accentVar}` : 'none', opacity: deck.isPlaying ? 1 : 0.35 }}
          />
          <h2 className="text-[10px] font-black tracking-[0.2em]" style={{ color: accentVar }}>
            DECK {deckName}
          </h2>
        </div>
        {deck.track && (
      ...
      <div className="flex-1 min-h-0 flex flex-col gap-1">
        {deck.track ? (
          <>
            ...
            <div className="flex-shrink-0">
              ...
              <div className="text-[9px] text-muted-foreground truncate">{deck.track.artist}</div>
              <div className="flex gap-1 mt-1 items-center flex-wrap">
                {/* BPM */}
                <span className={cn(
                  "text-[10px] font-mono px-1.5 py-0.5 rounded border",
                  deck.rate !== 1.0 ? "bg-amber-500/10 border-amber-500/30 text-amber-400" : "bg-background/40 border-white/10 text-foreground/70"
                )}>
                  {(deck.track.bpm * deck.rate).toFixed(1)}<span className="text-[7px] ml-0.5 opacity-50">BPM</span>
                </span>
                ...
                <span className="text-[9px] text-muted-foreground flex items-center gap-0.5">
                  <Zap className="w-2.5 h-2.5 text-amber-500" />{deck.track.energy.toFixed(1)}
                </span>
            ...
            <div className="flex justify-between text-[10px] font-mono flex-shrink-0 text-muted-foreground">
              <span className="text-foreground/80">{formatTime(deck.currentTime)}</span>
            ...
            <div className="h-5 w-full flex-shrink-0 opacity-75 border-b border-white/[0.03]">
              <canvas ref={canvasRef} width={200} height={20} className="w-full h-full" />
            </div>
            ...
            <div className="flex-shrink-0">
              ...
              >
                ...
                >
                  {waveform.map((h, i) => {
                    ...
                    let r = 0, g = 0, b = 0
                    if (h > 0.65) { r = 255; g = Math.floor(255 * (1 - h)); b = 50 }
                    else if (h > 0.35) { r = Math.floor(255 * h); g = 220; b = 0 }
                    else { r = 0; g = Math.floor(255 * h * 2); b = 255 }
                    ...
                    return (
                      <div key={i} className="flex-1 h-full flex items-center justify-center relative">
                        <div className="absolute w-full rounded-[1px]" style={{ height: `${Math.max(h * 95, 2)}%`, backgroundColor: outerColor }} />
                        <div className="absolute w-full rounded-[1px]" style={{ height: `${Math.max(Math.pow(h, 1.5) * 72, 2)}%`, backgroundColor: innerColor }} />
                      </div>
                    )
                  })}
                </div>
                ...
                {loop.start !== null && deck.track && (
                  <div
                    ...
                    style={{ left: `${(loop.start / deck.track.duration) * 100}%`, backgroundColor: '#00ff88', opacity: 0.9 }}
                  />
                )}
                {loop.end !== null && deck.track && (
                  <div
                    className="absolute top-0 bottom-0 w-[2px] z-[8]"
                    style={{ left: `${(loop.end / deck.track.duration) * 100}%`, backgroundColor: '#00ff88', opacity: 0.9 }}
                  />
                )}

                {/* Hot cue markers */}
                {deck.track && hotCues.map((cue, i) => cue !== null ? (
                  <div
                    key={i}
              ...
              <div className="flex justify-between text-[8px] mt-0.5 px-0.5">
                {['Intro', 'Verse', 'Chorus', 'Bridge', 'Outro'].map(s => {
                  const sectionIdx = Math.floor((deck.currentTime / deck.track!.duration) * 5)
                  const sections = ['Intro', 'Verse', 'Chorus', 'Bridge', 'Outro']
                  const isCurrent = sections[sectionIdx] === s
                  return (
                    <span key={s} className={isCurrent ? 'font-bold' : 'text-muted-foreground/30'} style={isCurrent ? { color: accentVar } : {}}>
                      {s}
                    </span>
                  )
                })}
          ...
          <div className="flex-1 flex flex-col justify-center items-center text-center">
            ...
            <div className="text-[10px] text-muted-foreground/40 font-medium tracking-wide">NO TRACK LOADED</div>
            <div className="text-[8px] text-muted-foreground/25 mt-0.5">Double-click library row to load</div>
          </div>
        )}
        ...
        <div className="mt-auto flex-shrink-0 space-y-1 pt-1.5 border-t border-white/[0.04]">
            ...
            {HOT_CUE_COLORS.map((color, i) => {
              ...
              return (
                <button
                  ...
                  onContextMenu={e => { e.preventDefault(); onHotCueClear(i) }}
                  title={isSet ? `Hot Cue ${i + 1}: ${formatTime(cue!)} — right-click to clear` : `Set Hot Cue ${i + 1}`}
                  className={cn(
                ...
                >
                  <div className="text-[9px] font-black leading-none" style={isSet ? { color } : { color: 'rgba(255,255,255,0.25)' }}>
                    {i + 1}
                  </div>
                  {isSet && (
                    <div className="text-[7px] font-mono leading-tight mt-0.5" style={{ color }}>
                      {formatTime(cue!)}
                    </div>
                  )}
                </button>
          ...
          <div className="flex items-center gap-1.5">
            ...
            />
            <span className={cn("text-[9px] w-9 text-right font-mono tabular-nums", deck.rate !== 1.0 ? "text-amber-400" : "text-muted-foreground/50")}>
