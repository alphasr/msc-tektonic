...
export default function TrackLibrary({ tracks, onLoadTrack, onRefresh, currentKey }: TrackLibraryProps) {
  ...
  const filtered = tracks.filter(t => {
    ...
    if (keyFilter && currentKey && !isKeyCompatible(currentKey, t.key)) return false;
    return true;
  }).sort((a, b) => {
    ...
    switch (sortKey) {
      ...
      case 'energy': va = a.energy; vb = b.energy; break;
  ...
  const SortIcon = ({ col }: { col: SortKey }) => {
    if (sortKey !== col) return <ArrowUpDown className='w-2.5 h-2.5 opacity-25' />;
    return sortDir === 'asc'
      ...
      <div className='flex-1 flex flex-col px-2 pt-1.5 pb-1 min-h-0 gap-1.5 overflow-hidden'>
        ...
        <div className='flex-shrink-0 flex items-center gap-1.5'>
          ...
          <button
            ...
            title={keyFilter ? 'Disable harmonic filter' : 'Filter harmonically compatible keys'}
            className={cn(
          ...
          <Button variant='ghost' size='sm' className='h-6 w-6 p-0 flex-shrink-0' onClick={onRefresh} title='Refresh library'>
        ...
        {activeTab === 'library' && (
          <div className='flex-1 min-h-0 overflow-hidden flex flex-col'>
            <div className='text-[9px] text-muted-foreground/40 px-1 mb-0.5 flex-shrink-0'>
              ...
              {keyFilter && currentKey && <span className='ml-2 text-primary/60'>♪ harmonic filter active</span>}
            </div>
              ...
              {tracks.length === 0 ? (
                <div className='flex items-center justify-center h-full'>
                  <div className='text-center'>
                    <div className='text-sm font-medium mb-1 text-foreground/60'>No tracks</div>
                    <div className='text-[10px] text-muted-foreground/50'>Upload tracks via the Analyze page.</div>
                  </div>
                </div>
              ) : filtered.length === 0 ? (
                <div className='flex items-center justify-center h-full'>
                  <div className='text-center'>
                    <div className='text-sm font-medium mb-1 text-foreground/60'>No matches</div>
                    <div className='text-[10px] text-muted-foreground/50'>Adjust your search or filters.</div>
                  </div>
                </div>
              ) : (
                <table className='w-full border-collapse'>
                  <thead className='sticky top-0 z-10' style={{ background: 'var(--card)' }}>
                    <tr className='border-b border-white/[0.04]'>
                      ...
                      <Th col='duration' label='Time' className='text-right w-14' />
                      <Th col='energy' label='Nrg' className='w-16' />
                      <th className='px-1 py-1 w-14 text-[9px] font-semibold text-muted-foreground'>Load</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filtered.map((track, idx) => {
                      const isSelected = selectedId === track.id;
                      const isCompatible = keyFilter && currentKey ? isKeyCompatible(currentKey, track.key) : false;
                      const energyColor =
                        track.energy > 7 ? '#e8334a' : track.energy > 4 ? '#ff9500' : '#1ac8e8';
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className='px-2 py-[4px] text-right'>
                            <span className='text-[10px] font-mono text-foreground/75'>{track.bpm}</span>
                          </td>
                          ...
                          <td className='px-2 py-[4px]'>
                            <span
                              ...
                              style={{ backgroundColor: getCamelotColor(track.key) }}
                            >
                              {track.key}
                            </span>
                          </td>
                          ...
                          {/* Duration */}
                          <td className='px-2 py-[4px] text-right'>
                            <span className='text-[10px] font-mono text-muted-foreground'>{formatDuration(track.duration)}</span>
                          </td>
                            ...
                            <div className='flex items-center gap-1'>
                              <div className='w-10 h-1 bg-white/10 rounded-full overflow-hidden'>
                                <div
                                  ...
                                  style={{ width: `${Math.min(track.energy * 10, 100)}%`, backgroundColor: energyColor }}
                                />
                              </div>
                              <span className='text-[9px] font-mono text-muted-foreground/60 w-5 text-right'>{track.energy.toFixed(1)}</span>
                            </div>
                          </td>
