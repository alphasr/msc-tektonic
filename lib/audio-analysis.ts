  ...
  if (!ffmpeg) {
    try {
      ...
      try {
        ...
        console.warn(
          'Could not load @ffprobe-installer/ffprobe, trying alternatives:',
          e
        );
...
import { extractFrequencyProfiles } from './frequency-analysis';
...
export interface AudioFeatures {
  ...
  phrases: number;
  waveform: number[];
  barFrequencies?: FrequencyProfile[]; // NEW: Frequency profiles per bar
  phraseFrequencies?: FrequencyProfile[]; // NEW: Frequency profiles per phrase
}
...
// Extract full audio samples for frequency analysis
async function extractFullAudioSamples(
  filePath: string,
  sampleRate: number
): Promise<number[]> {
  const tempWav = path.join(
    path.dirname(filePath),
    `freq_analysis_${createHash('md5').update(filePath).digest('hex')}.wav`
  );
  ...
  return new Promise((resolve, reject) => {
    ffmpegInstance(filePath)
      .audioFrequency(sampleRate)
      .audioChannels(1) // Mono for frequency analysis
      .audioCodec('pcm_s16le')
      .format('wav')
      .on('end', async () => {
        try {
...
function downsampleToBars(samples: number[], bars: number): number[] {
  ...
  for (let i = 0; i < bars; i++) {
    ...
    const rms = Math.sqrt(
      barSamples.reduce((sum, sample) => sum + sample * sample, 0) /
        barSamples.length
    );
...
export async function estimateKey(filePath: string): Promise<string> {
  ...
  return new Promise((resolve) => {
    ...
    const randomKey =
      camelotKeys[Math.floor(Math.random() * camelotKeys.length)];
    resolve(randomKey);
  });
}
...
export async function analyzeAudioFile(
  ...
  try {
    ...
    const phrases = Math.max(1, Math.floor(bars / barsPerPhrase));

    // Extract frequency profiles
    // First, extract full audio samples for frequency analysis
    const audioSamples = await extractFullAudioSamples(
      filePath,
      metadata.sampleRate
    );

    // Calculate frequency profiles per bar
    const barFrequencies = extractFrequencyProfiles(
      audioSamples,
      metadata.sampleRate,
      bars
    );

    // Calculate frequency profiles per phrase (aggregate bars)
    const phraseFrequencies: FrequencyProfile[] = [];
    const barsPerPhraseCalc = Math.floor(bars / phrases);

    for (let i = 0; i < phrases; i++) {
      const startBar = i * barsPerPhraseCalc;
      const endBar = Math.min(startBar + barsPerPhraseCalc, bars);

      // Average frequency profiles for bars in this phrase
      const phraseBars = barFrequencies.slice(startBar, endBar);
      if (phraseBars.length > 0) {
        const avgProfile: FrequencyProfile = {
          low:
            phraseBars.reduce((sum, p) => sum + p.low, 0) / phraseBars.length,
          mid:
            phraseBars.reduce((sum, p) => sum + p.mid, 0) / phraseBars.length,
          high:
            phraseBars.reduce((sum, p) => sum + p.high, 0) / phraseBars.length,
        };
        phraseFrequencies.push(avgProfile);
      }
    }
