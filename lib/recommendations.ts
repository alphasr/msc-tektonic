...
export interface TrackRecommendation {
  ...
  scores: {
    ...
    frequency: number; // Frequency compatibility (0-1) NEW
...
function tempoScore(bpmA: number, bpmB: number): number {
  ...
  if (diff <= 10) return 0.7; // Good
  if (diff <= 15) return 0.5; // Acceptable
  if (diff <= 20) return 0.3; // Challenging
...
export async function recommendTracks(
  ...
  for (const manifest of Object.values(manifests)) {
    ...
    try {
      ...
      const energy = energyTransitionScore(
        sourceSummary.energy,
        trackSummary.energy
      );

      // Texture similarity (average phrase vector)
      const avgTrackPhrase =
        trackFeatures.phraseVecs.length > 0
      ...
      const texture =
        avgSourcePhrase.length > 0 && avgTrackPhrase.length > 0
      ...
      const phrase = cosineSimilarity(
        ...
        trackFeatures.phraseVecs[
          Math.floor(trackFeatures.phraseVecs.length / 2)
        ] || []
      );

      // Frequency compatibility (average across phrases)
      let frequency = 0.5; // Default if frequency data not available
      if (sourceFeatures.phraseFrequencies && trackFeatures.phraseFrequencies) {
        const sourceFreqs = sourceFeatures.phraseFrequencies;
        const trackFreqs = trackFeatures.phraseFrequencies;

        // Compare middle phrases
        const sourceMid = sourceFreqs[Math.floor(sourceFreqs.length / 2)];
        const trackMid = trackFreqs[Math.floor(trackFreqs.length / 2)];

        if (sourceMid?.profile && trackMid?.profile) {
          frequency = frequencyCompatibilityScore(
            sourceMid.profile,
            trackMid.profile
          );
        }
      ...
      // Harmonic: 28%, Tempo: 23%, Energy: 18%, Texture: 13%, Phrase: 9%, Frequency: 9%
      const overall =
        0.28 * harmonic +
        0.23 * tempo +
        0.18 * energy +
        ...
        for (let i = 0; i < Math.min(5, trackFeatures.phraseVecs.length); i++) {
          ...
          if (similarity > 0.6) {
            similarSegments.push({
              position:
                (i / trackFeatures.phraseVecs.length) * trackSummary.duration,
              score: similarity,
              type: 'phrase',
            });
          }
