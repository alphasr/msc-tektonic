...
// Serialize read-modify-write cycles on manifests.json — concurrent callers
// (e.g. Promise.all over track imports) would otherwise drop each other's entries.
let manifestWriteLock: Promise<void> = Promise.resolve();

// Save manifest
export async function saveManifest(manifest: TrackManifest) {
  const write = manifestWriteLock.then(async () => {
    const manifests = await loadManifests();
    manifests[manifest.track_id] = manifest;
    await saveManifests(manifests);
  });
  manifestWriteLock = write.catch(() => {});
  await write;
}
